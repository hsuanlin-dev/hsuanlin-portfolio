    ...
    roles: [
      {
        ...
        highlights: {
          ...
          zh: [
            '在台北市政府多個數位平台專案中，擔任團隊主要的前端與無障礙諮詢窗口，將 Figma 設計稿轉化為 pixel-perfect、符合 WCAG 2.1 AA 標準的介面。',
            '在沒有正式規格或文件的情況下，主導兩個舊系統前端的重建工作，逆向解析國家公園入口網的 UI 慣例，在各自獨立的程式碼庫中獨立重現其視覺語言與元件模式。',
            '以唯一前端負責人的身分，從 0 到 1 推動原住民碳匯 ESG 平台，全程主導設計與前端實作——從定義視覺系統（SCSS 架構、色彩系統、響應式版面）到將利害關係人的需求轉化為可上線的程式碼。',
            '主導 EZSIGN 的品牌識別與 B2B 導入體驗——從零設計 Logo、色彩系統與 landing page，並將複雜的法律簽署流程簡化為引導式、著重信任感的 Wizard 流程。',
          ],
        },
      },
      {
